...
const [editingRequestId, setEditingRequestId] = useState(null);
  ...
  const handleSaveTeacher = async () => {
    if (newTeacher.name && newTeacher.subject && newTeacher.runk && newTeacher.login && newTeacher.password) {
      setIsLoading(true);
      let photoURL = '';
      ...
      try {
        ...
        setIsEditing(false);
        setNewTeacher({ name: '', subject: '', runk: '', login: '', password: '' });
        setPhotoFile(null);
                ...
                >
                  ...
                  <div className="compact-content">
                    ...
                    <FaEdit
                      ...
                      onClick={() => {
                        handleEditRequest(request.id);         // отправка запроса и локальное обновление
                        setIsEditingRequest(true);                    // показываем окно сразу
                        setEditingRequestId(request.id);       // если используешь это
                      }}
                    />
                  </div>
...
>
  Принять
            ...
            <button onClick={() => openDeleteReqModal(request.id, request.username)} style={{ background: '#d32f2f', color: '#fff' }}>Удалить</button>
            <button onClick={() => setIsEditingRequest(false)}>Закрыть</button>
          </div>
        );
      })()}
    </div>

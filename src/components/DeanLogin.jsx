...
const DeanLogin = () => {
  ...
  const [regData, setRegData] = useState({
    fio: '', subject: '', login: '', password: '', photo: null,
  });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const deanData = snapshot.val();
      if (!deanData) throw new Error('Профиль декана не найден.');
      navigate(`/teacher-profile/${user.uid}`, { state: { teacher: { ...deanData, id: user.uid } } });
    } catch (err) {
  ...
  const validateRegData = () => {
    const required = ['fio', 'subject', 'login', 'password'];
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await set(dbRef(database, `users/${user.uid}`), {
        username: deanData.fio,
        avatarUrl: deanData.photo || './default-image.png',
        role: 'dean',
        email: regData.login,
        subject: deanData.subject,
        cathedra: [
